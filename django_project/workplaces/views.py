    ...
    def form_valid(self, form):
        ...
        try:
            ...
            try:
                ...
                return redirect("workplaces:workplaces")

            except ValidationError as e:
                # Просто показываем общее сообщение об ошибке
                form.add_error(
                    None,
                    "Невозможно забронировать рабочее место. Проверьте правила размещения.",
                )
                return self.form_invalid(form)
...
class WorkplaceReleaseView(LoginRequiredMixin, UpdateView):
    ...
    template_name = "workplaces/clear_workplace.html"
...
class WorkplaceCreateView(LoginRequiredMixin, CreateView):
    ...
    template_name = "workplaces/add_workplace.html"
    success_url = reverse_lazy("workplaces:workplaces")

    def form_valid(self, form):
        messages.success(
            self.request, f"Рабочее место #{form.instance.desk_number} успешно создано"
        )
...
class WorkplaceDeleteView(LoginRequiredMixin, DeleteView):
    model = Workplace
    template_name = "workplaces/delete_workplace.html"
    success_url = reverse_lazy("workplaces:workplaces")

    def delete(self, request, *args, **kwargs):
        workplace = self.get_object()
        messages.success(request, f"Рабочее место #{workplace.desk_number} удалено")
...
def workplace_detail(request, pk):
    ...
    neighbors = Workplace.objects.filter(desk_number__in=neighbor_desks).select_related(
        "employee"
    )
